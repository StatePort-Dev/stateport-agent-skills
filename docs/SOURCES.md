# Public references

The initial repository boundary uses the Agent Skills specification and the
Paddle repository as structural references. No Paddle code or skill text is
copied into this source scaffold.

- Agent Skills specification: https://agentskills.io/specification
- Paddle structure: https://github.com/PaddleHQ/paddle-agent-skills
- GitHub CLI repository creation: https://cli.github.com/manual/gh_repo_create
- GitHub CLI repository settings: https://cli.github.com/manual/gh_repo_edit
- GitHub Actions workflow permissions:
  https://docs.github.com/en/rest/actions/permissions

Provider format documentation must be verified and recorded during each adapter
implementation. A structural reference is not evidence that StatePort works in
a particular host.
