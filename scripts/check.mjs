...
import { validateSkill, validateRegistry, validateRelativeLink, validatePublicGitHubRepositories, validateWorkflowLocalPaths } from './validate.mjs';
...
const repositoryFiles = new Set(all.map(relative));
...
for (const absolute of all) {
  ...
  add(name, validatePublicGitHubRepositories(content));
...
try {
  ...
  add('.github/workflows/ci.yml', validateWorkflowLocalPaths(workflow, repositoryFiles));
